...
async def get_todos(
    ...
    search: Optional[str] = Query(None, description="Search keywords in title and description"),
    due_date_from: Optional[date] = Query(None, description="Due date range - start date"),
    due_date_to: Optional[date] = Query(None, description="Due date range - end date"),
    
    # Sort parameters
    sort_by: str = Query("created_at", description="Sort field: created_at, updated_at, due_date, priority, title"),
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=TodoResponse, summary="Create Todo")
...
async def get_todo_statistics(
    ...
    try:
        ...
        logger.info(f"Generated todo statistics: total={stats['total']}, completed={stats['completed']}")
...
async def update_todo(
    ...
    try:
        # Update todo
        updated_todo = TodoService.update_todo(db, todo_id, todo_update.dict(exclude_unset=True))
