...
class CacheService:
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        result = await self._execute_with_fallback(_set_operation, key, value, effective_ttl)
        if result:
    ...
    async def cache_todo(self, todo_id: int, todo_data: Dict[str, Any], ttl: Optional[int] = None) -> bool:
        """
    ...
    async def get_todos_list(self, filters: Dict[str, Any], pagination: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    def _generate_list_key(self, filters: Dict[str, Any], pagination: Dict[str, Any]) -> str:
        """
        ...
        normalized_pagination = {k: v for k, v in sorted(pagination.items()) if v is not None}
        ...
        params_hash = hashlib.md5(params_str.encode()).hexdigest()[:12]  # Use first 12 chars
